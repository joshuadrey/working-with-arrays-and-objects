...
//Code Here
let evenNumbers = mixedNumbers.filter(function (nums, i, arr) {
  return nums % 2 === 0
})
...
//Code Here
let postTaxPrices = prices.map(nums => nums * 1.07)
...
//Code Here
let myStrongest = monstersInYourPocket.filter((cp, i, arr) => cp.CP > 200)
...
//   orderTotals.reduce((acc, curr) => [...acc, curr.price * (curr.tax + 1)], [])}
// function orderTotals(order) {
//   return orders.map(order => order.price + (order.price * order.tax))
// }
